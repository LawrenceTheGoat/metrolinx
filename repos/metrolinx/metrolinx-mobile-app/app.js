...
class VoiceTransitApp {
    ...
    async init() {
        try {
            ...
            this.realGTFSAPI = new RealGTFSAPI();
            ...
            // Update status - Mandarin only
            this.updateStatus('你好！我准备好帮助您解答交通问题。按住按钮说话');
    ...
    async processVoiceInput(result) {
        ...
        try {
            ...
            // Check if this is a language preference setting (exact "你好" or English greetings)
            if (this.isLanguagePreference(result.transcript)) {
                await this.handleLanguagePreference(result);
                return;
            }
            
            // Check if this starts with Chinese greeting but has more content - treat as Chinese search
            if (this.isChineseInput(result.transcript)) {
                console.log('Input starts with Chinese greeting, treating as Chinese search');
                result.language = 'zh'; // Override language detection
            }
            
            // Update status
            this.updateStatus('Processing your request...');
            
            // Query the API with detected language
            console.log('App Processing - Calling API with transcript:', result.transcript);
            console.log('App Processing - Calling API with language:', result.language);
            const apiResponse = await this.realGTFSAPI.processVoiceQuery(result.transcript, result.language);
            console.log('App Processing - API response:', apiResponse);
            
            // Use the detected language from speech recognition for response
            let responseLanguage = result.language;
            
            console.log(`User spoke in: ${result.language}`);
            console.log(`API response message: "${apiResponse.message}"`);
            
            // Always use the user's detected language for speech output
            // The API should already return content in the correct language
            if (result.language === 'zh') {
                responseLanguage = 'zh';
                console.log('User spoke Chinese, using Chinese voice for response');
            } else {
                responseLanguage = 'en';
                console.log('User spoke English, using English voice for response');
            }
            
            console.log('Final response language determined:', responseLanguage);
            
            // Generate response text using the determined language
            const responseData = this.formatResponse(apiResponse, responseLanguage);
            ...
            // Speak the response with the determined language
            this.updateButtonState('pause');
            this.updateStatus('Playing response. Click to pause');
            
            await this.speechHandler.speak(speechText, responseLanguage);
    ...
    formatResponse(apiResponse, language = 'en') {
        ...
        if (apiResponse.success) {
            // For successful responses, just return the message
            return apiResponse.message;
        } else {
            // For error responses, return the error message
            return apiResponse.message || 'Sorry, I couldn\'t process your request.';
        }
    }
    ...
    /**
     * Show transcript in UI - Bilingual support
     */
    showTranscript(text, language = null, isInterim = false) {
        // Use innerHTML to properly display Chinese characters
        this.elements.transcriptText.innerHTML = text.replace(/\n/g, '<br>');
        
        // Show appropriate language indicator based on detected language
        if (language === 'zh') {
            this.elements.detectedLanguage.innerHTML = `Detected: 🇨🇳 中文`;
            this.elements.languageIndicator.innerHTML = `🇨🇳 中文`;
        } else if (language === 'en') {
            this.elements.detectedLanguage.innerHTML = `Detected: 🇺🇸 English`;
            this.elements.languageIndicator.innerHTML = `🇺🇸 English`;
        } else {
            // Default fallback
            this.elements.detectedLanguage.innerHTML = `Detected: 🌐 Auto`;
            this.elements.languageIndicator.innerHTML = `🌐 Auto`;
        }
    ...
    /**
     * Check if the input is a language preference statement or leading word
     */
    isLanguagePreference(text) {
        const lowerText = text.toLowerCase().trim();
        const originalText = text.trim();
        
        // Check for exact "你好" - this should set language preference only
        if (originalText === '你好' || lowerText === 'nihao' || lowerText === 'ni hao') {
            return true;
        }
        
        // Check for direct language preferences
        if (lowerText === '中文' || lowerText === 'english' || lowerText === 'chinese' || 
            lowerText === 'english language' || lowerText === 'speak english' || lowerText === 'zhongwen') {
            return true;
        }
        
        // Check for English greeting words that set language preference
        const englishLeadingWords = ['hello', 'hi', 'hey', 'good morning', 'good afternoon', 'good evening'];
        
        for (const word of englishLeadingWords) {
    ...
    /**
     * Check if the input starts with Chinese greeting and should be treated as Chinese
     */
    isChineseInput(text) {
        const originalText = text.trim();
        
        // If it starts with "你好", treat as Chinese input
        if (originalText.startsWith('你好')) {
            return true;
        }
        
        // Check for other Chinese greeting variations
        const chineseGreetings = ['您好', 'nihao', 'ni hao', 'neehow', 'knee how', 'nee how'];
        const lowerText = text.toLowerCase().trim();
    ...
    /**
     * Handle language preference setting and leading words
     */
    async handleLanguagePreference(result) {
        try {
            ...
            // Handle Chinese leading words or preferences
            if (lowerText === '中文' || lowerText === 'chinese' || lowerText === 'zhongwen' ||
                lowerText.startsWith('你好') || lowerText.startsWith('nihao') || lowerText.startsWith('ni hao') ||
                lowerText.startsWith('neehow') || lowerText.startsWith('knee how') || lowerText.startsWith('nee how') ||
                lowerText.startsWith('您好')) {
                responseText = '你好！我现在将用中文为您提供帮助。请问您需要什么交通信息？';
                language = 'zh';
            }
            // Handle English leading words
            else if (lowerText.startsWith('hello') || lowerText.startsWith('hi') || lowerText.startsWith('hey') ||
                     lowerText.startsWith('good morning') || lowerText.startsWith('good afternoon') || 
                     lowerText.startsWith('good evening') || lowerText === 'english' || 
                     lowerText === 'english language' || lowerText === 'speak english') {
                responseText = 'Hello! I\'ll now assist you in English. What transit information can I help you with?';
                language = 'en';
            }
...
document.addEventListener('visibilitychange', () => {
    if (document.hidden && window.voiceTransitApp) {
        ...
        // Add test functionality for Chinese search
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (testButton && testInput) {
                testButton.addEventListener('click', async () => {
                    ...
                    if (!query) {
                        testInput.value = '联合站到金莺站';
                        return;
                    }
                    
                    if (window.voiceTransitApp) {
                        // Simulate a voice input result
                        const mockResult = {
                            transcript: query,
                            language: query.match(/[\u4e00-\u9fff]/) ? 'zh' : 'en'
                        };
                        
                        console.log('Testing with mock result:', mockResult);
                        await window.voiceTransitApp.processVoiceInput(mockResult);
                    }
                });
